  ...
  const t = useCallback(
    (key: TranslationKey): string => {
      ...
      if (language === "system") {
        ...
        const codeFromLanguageCode = locale?.languageCode;
        const codeFromLanguageTag = locale?.languageTag
          ?.split("-")?.[0]
          ?.toLowerCase();
        const deviceLanguage = codeFromLanguageCode || codeFromLanguageTag;
